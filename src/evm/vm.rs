...
use revm_interpreter::{
    BytecodeLocked, CallContext, CallScheme, Contract, Gas, InstructionResult,
    InstructionResult::ControlLeak, Interpreter, Memory, Stack,
};
...
use crate::{
    evm::{
        bytecode_analyzer,
        host::{
            FuzzHost, CMP_MAP, WRITE_MAP, READ_MAP, STATE_CHANGE, TXN_WAYPOINT_MAP,
        },
    ...
    SC: Scheduler<State = EVMFuzzState> + Clone + 'static,
{
    ...
    pub fn execute_from_pc(
        ...
        // Initial setups
        if cleanup {
            self.host.waypoint_changed = false;
    ...
    fn execute_abi(
        ...
        #[cfg(feature = "real_balance")]
        {
            ...
            if tx_value > EVMU256::ZERO {
                let caller_balance = *vm_state
                    .get_balance(&input.get_caller())
                    .unwrap_or(&EVMU256::ZERO);
                let contract_balance = *vm_state
...
mod tests {
    ...
    use crate::{
        evm::{
            host::{FuzzHost, TXN_WAYPOINT_MAP},
    ...
    fn test_fuzz_executor() {
        ...
        */
        ...
        for i in 0..MAP_SIZE {
            know_map[i] = unsafe { TXN_WAYPOINT_MAP[i] };
            unsafe { TXN_WAYPOINT_MAP[i] = 0 };
        }
        ...
        for i in 0..MAP_SIZE {
            let hit = unsafe { TXN_WAYPOINT_MAP[i] };
