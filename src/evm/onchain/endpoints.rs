...
impl OnChainConfig {
    fn get_pair(&mut self, token: &str, network: &str, is_pegged: bool) -> Vec<PairData> {
        ...
        let url_resp = reqwest::blocking::get(url);
        if url_resp.is_err() {
            return vec![];
        }
        let resp: Value = url_resp.unwrap().json().unwrap();
    ...
    fn fetch_reserve(&self, pair: &str) -> (String, String) {
        let result = {
            ...
            debug!("fetching reserve for {pair} {} {params}", self.block_number);
    ...
    pub fn fetch_transaction_by_hash(&mut self, tran_hash: String) -> Option<EVMInput> {
        ...
        match resp {
            Some(resp) => {
                let caller =
                    EVMAddress::from_str(resp.get("from").unwrap().as_str().unwrap()).unwrap();
                let contract =
                    EVMAddress::from_str(resp.get("to").unwrap().as_str().unwrap()).unwrap();
                let mut txn = EVMInput {
                    ...
                    sstate_idx: 0,
                    txn_value: Some(EVMU256::from(
                        EVMU256::from_str(resp.get("value").unwrap().as_str().unwrap()).unwrap(),
                    )),
                    step: false,
                    env: Default::default(),
                ...
                {
                    let balance_caller = self.get_balance(caller);
                    let balance_contract = self.get_balance(contract);
                    txn.get_state_mut().set_balance(caller, balance_caller);
                    txn.get_state_mut().set_balance(contract, balance_contract);
                }

                let abi = self.fetch_abi(txn.contract);
                let txn_data = resp.get("input").unwrap().as_str().unwrap();
                ...
                // nasty implementation (todo@a3yip6)
                match txn.data {
                    None => {
                        let implementation_slot =
                            "0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc";
                        let implementation_addr = convert_u256_to_h160(self.get_contract_slot(
                            txn.contract,
                            EVMU256::from_str(implementation_slot).unwrap(),
                            false,
                        ));
                        let abi = self.fetch_abi(implementation_addr);
                        if abi.is_some() {
                            let abis = ContractLoader::parse_abi_str(&abi.unwrap());
                            for abi in abis {
                                if abi.function != &txn_func[..] {
                                ...
                                abi_instance.set_bytes(txn_para);
                                txn.data = Some(abi_instance);
                                break;
                            }
                        }
