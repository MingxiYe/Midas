...
pub fn generate_test<T: SolutionTx>(solution: String, inputs: Vec<T>) {
    ...
    if handlebars
        .register_template_string("foundry_test", TEMPLATE)
        .is_err()
    {
...
impl<T: SolutionTx> From<&T> for Tx {
    fn from(input: &T) -> Self {
        let (is_borrow, mut liq_percent, swap_data) =
            (input.is_borrow(), input.liq_percent(), input.swap_data());
...
impl TemplateArgs {
    pub fn new(solution: String, trace: Vec<Tx>) -> Result<Self, String> {
        ...
        let include_interface = trace.iter().any(|x| {
            !x.raw_code.is_empty() || x.buy_type == BuyType::Buy || x.sell_type == SellType::Sell
        });
