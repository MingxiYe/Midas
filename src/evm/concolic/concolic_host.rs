...
lazy_static! {
    ...
    pub static ref ALL_WORKER_THREADS: Mutex<Vec::<std::thread::JoinHandle<()>>> =
        Mutex::new(Vec::new());
}
    ...
    pub fn solve(&mut self, optimistic: bool) -> Vec<Solution> {
        ...
        match result {
            z3::SatResult::Sat => {
                ...
                let origin_addr =
                    EVMAddress::from_slice(&hex::decode(&origin.as_str()[26..66]).unwrap());
                vec![Solution {
                    input: input_bytes,
                    caller: caller_addr,
                    origin: origin_addr,
        ...
        let bv: Vec<Option<Box<Expr>>> = match *interp.instruction_pointer {
            ...
            0x57 => {
                ...
                if self.call_depth > MAX_CALL_DEPTH {
                    debug!(
                        "[concolic] skip solving due to call depth: {}",
                        self.call_depth
                    );
                    need_solve = false;
                } else {
                    ...
                    match SOURCE_MAP_PROVIDER
                        .lock()
                        .unwrap()
                        .get_source_code(address, pc)
                    {
