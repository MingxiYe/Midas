...
use crate::{
    ...
    input::{ConciseSerde, HasPostState, SolutionTx, VMInputT},
...
pub struct EVMInput {
    ...
    /// The original post state in the historical txn
    #[serde(skip_deserializing)]
    pub post_state: Option<EVMState>,

    /// The original caller in the historical txn
    pub init_caller: Option<EVMAddress>,
}
...
pub struct ConciseEVMInput {
    ...
    /// Input data in String, in case no ABI found
    // #[cfg(feature = "debug")]
...
impl HasPostState<EVMState> for EVMInput {
    fn is_post_state(&self) -> bool {
        self.post_state.is_some()
    }

    fn set_post_state(&mut self, post_state: EVMState) {
