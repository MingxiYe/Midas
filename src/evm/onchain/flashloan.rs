...
use crate::evm::oracles::icy_bug::IcyBugOracle;
...
/// currently we implement Flashloan in a nasty way
/// flashloan oracle & icy oracle should not exist in oracles at the same time
    ...
    VS: VMStateT,
{
    ...
    #[cfg(feature = "flashloan_v2")]
    pub icy_oracle: Rc<RefCell<IcyBugOracle>>,
}
    ...
    VS: VMStateT,
{
    ...
    pub fn new(
        ...
        icy_oracle: Rc<RefCell<IcyBugOracle>>,
    ) -> Self {
        Self {
            phantom: PhantomData,
            oracle: price_oracle,
    ...
    pub fn on_contract_insertion(
        ...
        // check abi_signatures_token is subset of abi.name
        {
            ...
            if oracle.is_ok() {
                ...
                error!("Ignoring token {:?} during flashloan", addr);
            }

            let mut oracle = self.icy_oracle.deref().try_borrow_mut();
            // avoid delegate call on token -> make oracle borrow multiple times
            if oracle.is_ok() {
                if abi_signatures_token.iter().all(|x| abi_names.contains(x)) {
                    oracle.unwrap().register_token(
                ...
                error!("Ignoring token {:?} during icyoracle", addr);
            }
        }
