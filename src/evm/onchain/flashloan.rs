...
use bytes::Bytes;
use libafl::{
    corpus::{Corpus, HasTestcase, Testcase},
    inputs::Input,
...
use crate::{
    evm::{
        ...
        types::{
            convert_u256_to_h160, EVMAddress, EVMExecutionResult, EVMFuzzState, EVMU256, EVMU512,
        },
...
pub struct Flashloan {
    ...
    nft_address: HashSet<EVMAddress>,
    ...
    SC: Scheduler<State = S> + Clone,
{
    ...
    let mut tc =
        Testcase::new(input.clone().as_any().downcast_ref::<I>().unwrap().clone()) as Testcase<I>;
...
impl Flashloan {
    ...
    pub fn on_contract_insertion(
        ...
        // if the contract is nft
        if abi_signatures_nft.iter().all(|x| abi_names.contains(x)) {
            self.nft_address.insert(*addr);
            debug!("NFT detected @ address {:?}", addr);
            let oracle = self.flashloan_oracle.deref().try_borrow_mut();
            if oracle.is_ok() {
                oracle.unwrap().register_nft(*addr);
            }
        }
...
impl Flashloan {
    pub fn analyze_call(&self, input: &EVMInput, flashloan_data: &mut FlashloanData) {
        ...
        let is_replay = input.init_caller.is_some() && input.init_caller.clone().unwrap() == input.caller;
        if input.get_txn_value().is_some() && !is_replay {
            flashloan_data.owed += EVMU512::from(input.get_txn_value().unwrap()) * scale!();
        } else if input.get_txn_value().is_some() && is_replay {
            flashloan_data.targeted_caller_owed += EVMU512::from(input.get_txn_value().unwrap()) * scale!();
        }
        let addr = input.get_contract();
        // dont care if the call target is not erc20
        if self.erc20_address.contains(&addr) || self.nft_address.contains(&addr) {
    ...
    SC: Scheduler<State = EVMFuzzState> + Clone,
{
    unsafe fn on_step(
        ...
        let init_caller = s.testcase(
                s.get_current_input_idx().into()
            ).unwrap().input().as_ref().unwrap().init_caller;
        ...
        } else if value_transfer > EVMU256::ZERO && init_caller.is_some() && init_caller.unwrap() == call_target {
            host.evmstate.flashloan_data.targeted_caller_earned += EVMU512::from(value_transfer) * scale!();
        }

        let call_target: EVMAddress = convert_u256_to_h160(interp.stack.peek(1).unwrap());
        if self.erc20_address.contains(&call_target) || self.nft_address.contains(&call_target) {
            host.evmstate
                .flashloan_data
...
pub struct FlashloanData {
    ...
    pub owed: EVMU512,
    pub earned: EVMU512,
    pub targeted_caller_owed: EVMU512,
    pub targeted_caller_earned: EVMU512,
...
impl FlashloanData {
    pub fn new() -> Self {
        Self {
            ...
            targeted_caller_owed: Default::default(),
            targeted_caller_earned: Default::default(),
