...
use libafl::{
    ...
    state::{
        HasClientPerfMonitor, HasCorpus, HasExecutions, HasLastReportTime, HasMaxSize,
        HasNamedMetadata, HasRand, HasSolutions, State,
    },
    Error,
};
use libafl_bolts::{
    ...
    CI: Serialize + DeserializeOwned + Debug + Clone + ConciseSerde,
{
    ...
    /// The input corpus
    txn_corpus: TxnInMemoryCorpus<VI>,
    ...
    CI: Serialize + DeserializeOwned + Debug + Clone + ConciseSerde,
{
    ...
    pub fn new(lparam_seed: u64, work_dir: String) -> Self {
        ...
        Self {
            ...
            txn_corpus: TxnInMemoryCorpus::new(format!("{}/txn_corpus", work_dir)),
            solutions: OnDiskCorpus::new(Path::new(&format!("{}/solutions", work_dir))).unwrap(),
            executions: 0,
    ...
    CI: Serialize + DeserializeOwned + Debug + Clone + ConciseSerde,
{
    /// Create a new [`FuzzState`] with default values
    fn default() -> Self {
        Self::new(0, String::from("work_dir"))
    }
}
...
where
    ...
    CI: Serialize + DeserializeOwned + Debug + Clone + ConciseSerde,
{
    type Corpus = TxnInMemoryCorpus<VI>;
    ...
    CI: Serialize + DeserializeOwned + Debug + Clone + ConciseSerde,
{
    ...
    fn get_next_call(&mut self) -> Option<(EVMAddress, BoxedABI)> {
        ...
        let sig = self.interesting_signatures[self
            .rand_generator
            .below(self.interesting_signatures.len() as u64)
            as usize];
