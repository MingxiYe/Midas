...
lazy_static! {
    pub static ref SOURCE_MAP_PROVIDER: Mutex<SourceMapProvider> =
        Mutex::new(SourceMapProvider::default());
    pub static ref MULTILINE_REGEX: Regex =
        Regex::new(r"^(library|contract|function)(.|\n)*\}$").unwrap();
}
...
impl SourceMapProvider {
    ...
    pub fn get_raw_source_map_info(
        &self,
        address: &EVMAddress,
        pc: usize,
    ) -> Option<RawSourceMapInfo> {
        if self.has_source_map(address) {
            self.source_maps
                .get(address)
                .unwrap()
...
impl SourceMapItem {
    ...
    pub fn _set_source_code_from_file(&mut self, filename: String) {
        ...
        let source_code = file_content
            [self.raw_info.offset..self.raw_info.offset + self.raw_info.length]
            .to_string();
        self.source_code = Some(source_code);
    }
}
