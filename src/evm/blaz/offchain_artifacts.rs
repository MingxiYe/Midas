...
impl OffChainArtifact {
    ...
    pub fn from_build_info_file(file: String) -> Result<Vec<Self>, Box<dyn Error>> {
        let json = std::fs::read_to_string(file)?;
        Self::from_build_info_json(json)
    }
    ...
    pub fn from_build_info_json(json: String) -> Result<Vec<Self>, Box<dyn Error>> {
        let arr = serde_json::from_str::<Value>(&json)?;
        let mut artifacts = vec![];

        let sources_json = arr["input"]["sources"].as_object().expect("fail to get sources");
        let mut sources = Vec::new();
        for (filename, code) in sources_json {
            let code = code.to_string();
            sources.push((filename.to_string(), code));
        }

        let remappings = arr["input"]["settings"]["remappings"].as_array();
        let mut source_map_replacements = Vec::new();
        if remappings.is_some() {
            for item in remappings.unwrap() {
                let item = item.as_str().expect("failed to parse remappings").to_string();
                let map = item.rsplit_once("=").unwrap();
                source_map_replacements.push((map.0.to_string(), map.1.to_string()));
            }
        }

        let mut contracts = HashMap::new();
        let all_contracts = arr["output"]["contracts"].as_object().expect("fail to parse contracts");
        for (file_name, contract_dict) in all_contracts {
            for (contract_name, json) in contract_dict.as_object().expect("fail to parse contract dict") {
                let abi = json["abi"].to_string();
                let deploy_bytecode = json["evm"]["deployedBytecode"]["object"].as_str().expect("fail to get deployed bytecode");
                let deploy_bytecode = Bytes::from(hex::decode(deploy_bytecode).expect("fail to decode bytecode"));
                let source_map = json["evm"]["deployedBytecode"]["sourceMap"].as_str().expect("failed to get source map");
                contracts.insert(
                    ...
                    ContractArtifact {
                        ...
                        source_map_replacements: source_map_replacements.clone()
                    }
                );
            }
        }
