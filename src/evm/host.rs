...
use crate::evm::onchain::flashloan::{register_borrow_txn, Flashloan};
use bytes::Bytes;
...
use crate::evm::types::{
    as_u64, bytes_to_u64, generate_random_address, is_zero, EVMAddress, EVMU256, EVMU512,
};
...
use revm_interpreter::{
    ...
    InstructionResult, Interpreter, OpCode, SelfDestructResult,
};
...
// combine path & branch (JUMPI) coverage
pub static mut JMP_OP_MAP: [u8; MAP_SIZE] = [0; MAP_SIZE];

// tokenflow
pub const TOKEN_FLOW_LEN: usize = 4;
pub static mut TOKEN_FLOW_MAP: [[EVMU512; TOKEN_FLOW_LEN]; MAP_SIZE] =
    [[EVMU512::ZERO; TOKEN_FLOW_LEN]; MAP_SIZE];
pub static mut TRANSFER_DIFF_MAP: [u8; MAP_SIZE] = [0; MAP_SIZE];
    ...
    SC: Scheduler<State = S> + Clone,
{
    fn step(&mut self, interp: &mut Interpreter, state: &mut S) -> InstructionResult {
        unsafe {
            // debug!("pc: {:#x}; opcode: {}", interp.program_counter(), OpCode::try_from_u8(*interp.instruction_pointer).unwrap().as_str());
            // debug!("opcode: {:#04x}", *interp.instruction_pointer);
            ...
            match *interp.instruction_pointer {
                ...
                0x57 => {
                    ...
                    if JMP_OP_MAP[idx] == 0 {
                        JMP_OP_MAP[idx] = 1;
                        self.jmp_coverage_changed = true;
                    }
                }
                ...
                0xf1 | 0xf4 => {
                    ...
                    if arg_offset + arg_size <= interp.memory.len() {
                        ...
                        let mut token_flow: EVMU512;
                        if arg_size >= 68 && input[0..4] == [0xa9, 0x05, 0x9c, 0xbb] {
                            token_flow = EVMU512::try_from_be_slice(&input[4..68]).unwrap();
                        } else {
                            token_flow = EVMU512::from(fast_peek!(1)).saturating_shl(256)
                                + EVMU512::from(fast_peek!(2));
                        }
                        let mut target_idx = TOKEN_FLOW_LEN;
                        for i in 0..TOKEN_FLOW_LEN {
                            if unsafe { TOKEN_FLOW_MAP[pc % MAP_SIZE][i] } == token_flow {
                                break;
                            } else if unsafe { TOKEN_FLOW_MAP[pc % MAP_SIZE][i] }
                                == EVMU512::from(0)
                            {
                                target_idx = i;
                                break;
                        ...
                        if target_idx != TOKEN_FLOW_LEN {
                            unsafe { TOKEN_FLOW_MAP[pc % MAP_SIZE][target_idx] = token_flow };
                            unsafe { TRANSFER_DIFF_MAP[pc % MAP_SIZE] += 1 };
                        }
                    }
                }
