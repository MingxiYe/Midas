...
use libafl::{
    ...
    prelude::Testcase, schedulers::Scheduler, Error,
};
    ...
    SC: Scheduler<State = EVMFuzzState> + Clone + 'static,
{
        ...
        OT: ObserversTuple<EVMFuzzState>,
    {
        ...
        let is_interesting = match self
            .inner_feedback
            .is_interesting(state, manager, input, observers, exit_kind)
        {
            Ok(true) => {
                Ok( !state.get_execution_result().reverted || self.evm_executor.borrow().host.waypoint_changed )
            },
            Ok(false) => Ok(false),
            Err(e) => Err(e),
        };
