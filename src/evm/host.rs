...
use revm_interpreter::{
    ...
    return_ok, BytecodeLocked, CallContext, CallInputs, CallScheme, Contract, CreateInputs, Gas,
    Host, InstructionResult,
    InstructionResult::{Continue, ControlLeak, Revert},
    Interpreter, SelfDestructResult,
};
use revm_primitives::{
    BerlinSpec, Bytecode, ByzantiumSpec, Env, FrontierSpec, HomesteadSpec, IstanbulSpec,
    LatestSpec, LondonSpec, MergeSpec, PetersburgSpec, ShanghaiSpec, SpecId, SpuriousDragonSpec,
    TangerineSpec, B256,
};
use tracing::debug;
...
use super::{
    middlewares::cheatcode::{
        ExpectedCallData, ExpectedCallTracker, ExpectedCallType, ExpectedEmit, ExpectedRevert,
        Prank, ERROR_PREFIX, REVERT_PREFIX,
    },
    types::{convert_u256_to_h160, EVMFuzzState},
...
use crate::{
    evm::{
        ...
        vm::{
            is_reverted_or_control_leak, EVMState, SinglePostExecution, IN_DEPLOY,
            IS_FAST_CALL_STATIC,
        },
    },
    generic_vm::vm_executor::MAP_SIZE,
    handle_contract_insertion, invoke_middlewares,
    state::{HasCaller, HasHashToAddress},
    state_input::StagedVMState,
};

// for validity and performance waypoint
pub const CONTEXT_LEN: usize = 4;
pub static mut CONTEXT_MAP: [[EVMAddress; CONTEXT_LEN]; MAP_SIZE] =
    [[EVMAddress::zero(); CONTEXT_LEN]; MAP_SIZE];
pub static mut TXN_WAYPOINT_MAP: [u8; MAP_SIZE] = [0; MAP_SIZE];
...
pub static mut WAYPOINT_CHANGED: u32 = 0;
    ...
    SC: Scheduler<State = EVMFuzzState> + Clone,
{
    ...
    pub fn run_inspect(
        ...
        match self.spec_id {
            SpecId::LATEST => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, LatestSpec>(self, state)
            }
            SpecId::FRONTIER => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, FrontierSpec>(self, state)
            }
            SpecId::HOMESTEAD => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, HomesteadSpec>(self, state)
            }
            SpecId::TANGERINE => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, TangerineSpec>(self, state)
            }
            SpecId::SPURIOUS_DRAGON => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, SpuriousDragonSpec>(self, state)
            }
            SpecId::BYZANTIUM => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, ByzantiumSpec>(self, state)
            }
            SpecId::CONSTANTINOPLE | SpecId::PETERSBURG => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, PetersburgSpec>(self, state)
            }
            SpecId::ISTANBUL => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, IstanbulSpec>(self, state)
            }
            SpecId::MUIR_GLACIER | SpecId::BERLIN => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, BerlinSpec>(self, state)
            }
            SpecId::LONDON => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, LondonSpec>(self, state)
            }
            SpecId::MERGE => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, MergeSpec>(self, state)
            }
            SpecId::SHANGHAI => {
                interp.run_inspect::<EVMFuzzState, FuzzHost<SC>, ShanghaiSpec>(self, state)
            }
    ...
    fn check_expected_revert(
        ...
        if actual_reason.len() >= 4
            && matches!(
                actual_reason[..4].try_into(),
                Ok(ERROR_PREFIX | REVERT_PREFIX)
            )
        {
    ...
    SC: Scheduler<State = EVMFuzzState> + Clone,
{
    fn step(&mut self, interp: &mut Interpreter, state: &mut EVMFuzzState) -> InstructionResult {
        unsafe {
            ...
            // debug!("{:#02x?}", *interp.instruction_pointer);
            ...
            match *interp.instruction_pointer {
                ...
                0x57 => {
                    ...
                    TXN_WAYPOINT_MAP[idx] = TXN_WAYPOINT_MAP[idx].saturating_add(1);
                    ...
                    #[cfg(feature = "txn_waypoint")]
                    {
                        let idx = (_pc) % MAP_SIZE;
                        if TXN_WAYPOINT_MAP[idx] == 0 {
                            TXN_WAYPOINT_MAP[idx] = 1;
                ...
                0x55 => {
                        ...
                        .contains(&(interp.contract.address, pc))
                    {
                        ...
                        let idx = interp.program_counter() % MAP_SIZE;
                        TXN_WAYPOINT_MAP[idx] = if value_changed { 1 } else { 0 };
                ...
                0xf1 | 0xf4 => {
                    ...
                    #[cfg(feature = "txn_waypoint")]
                    {
                        let arg_offset = as_u64(fast_peek!(offset_of_ret_size - 3)) as usize;
                        let arg_size = as_u64(fast_peek!(offset_of_ret_size - 2)) as usize;
                        if arg_offset + arg_size <= interp.memory.len() {
                            let input = interp.memory.get_slice(arg_offset, arg_size);
                            if arg_size >= 68
                                && (input[0..4] == [0xa9, 0x05, 0x9c, 0xbb]
                                    || input[0..4] == [0x23, 0xb8, 0x72, 0xdd])
                            {
                                let caller = interp.contract().address;
                                let mut target_idx = CONTEXT_LEN;
                                for i in 0..CONTEXT_LEN {
                                    if CONTEXT_MAP[pc % MAP_SIZE][i] == caller {
                                        break;
                                    } else if CONTEXT_MAP[pc % MAP_SIZE][i] == EVMAddress::zero() {
                                        target_idx = i;
                                        break;
                                    }
                                }
                                if target_idx != CONTEXT_LEN {
                                    CONTEXT_MAP[pc % MAP_SIZE][target_idx] = caller;
                                    TXN_WAYPOINT_MAP[pc % MAP_SIZE] += 1;
                                    self.waypoint_changed = true;
                                }
                            }
                        }
    ...
    fn code_hash(&mut self, address: EVMAddress) -> Option<(B256, bool)> {
        match self.code.get(&address) {
            Some(_code) => Some((
                B256::from_str("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff").unwrap(),
                true,
            )),
            None => Some((
