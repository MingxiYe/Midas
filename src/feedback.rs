...
use crate::state::{
    HasCaller, HasCurrentInputIdx, HasExecutionResult, HasInfantStateState, HasItyState,
    InfantStateState,
};
...
use libafl::prelude::{Feedback, HasMetadata, HasRand, UsesInput};
...
/// IcyFeedback is a wrapper around a set of oracles and producers.
/// It executes the producers and then oracles after each successful execution. If any of the oracle
/// returns true, then it returns true and report a vulnerability found.
/// It extends OracleFeedback with init state
pub struct IcyFeedback<'a, VS, Addr, Code, By, Loc, SlotTy, Out, I, S: 'static, CI>
where
    I: VMInputT<VS, Loc, Addr, CI>,
    VS: Default + VMStateT,
    Addr: Serialize + DeserializeOwned + Debug + Clone,
...
impl<'a, VS, Addr, Code, By, Loc, SlotTy, Out, I, S, CI> Debug
    for IcyFeedback<'a, VS, Addr, Code, By, Loc, SlotTy, Out, I, S, CI>
where
    I: VMInputT<VS, Loc, Addr, CI>,
    VS: Default + VMStateT,
    Addr: Serialize + DeserializeOwned + Debug + Clone,
...
{
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IcyFeedback").finish()
    }
}
...
impl<'a, VS, Addr, Code, By, Loc, SlotTy, Out, I, S, CI>
    IcyFeedback<'a, VS, Addr, Code, By, Loc, SlotTy, Out, I, S, CI>
where
    S: State
        + HasClientPerfMonitor
        + HasExecutionResult<Loc, Addr, VS, Out, CI>
...
where
    ...
    CI: Serialize + DeserializeOwned + Debug + Clone + ConciseSerde,
{
    /// since IcyFeedback is an extension of OracleFeedback
    /// we also don't need to do initialization either
    fn init_state(&mut self, _state: &mut S) -> Result<(), Error> {
        Ok(())
    }
        ...
        OT: ObserversTuple<S>,
    {
        ...
        let mut is_any_bug_hit = false;
        // we ignore init_ctx, as it should success
        let has_post_exec = oracle_ctx
            .fuzz_state
            .get_execution_result()
            .new_state
