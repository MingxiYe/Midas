...
use crate::feedback::IcyFeedback;
...
use libafl::prelude::{
    Corpus, Executor, ExitKind, Feedback, ObserversTuple, SimpleEventManager, SimpleMonitor,
    StdScheduler,
};
    ...
    fn minimize(
        ...
        txs.extend(
            input
                .transactions
                .iter()
                .map(|ci| ci.to_input(last_sstate.clone())),
        );
        assert!(txs.len() >= 1);
...
type EVMIcyFeedback<'a> = IcyFeedback<
    'a,
    EVMState,
    revm_primitives::B160,
    Bytecode,
    ...
    > for EVMMinimizer
{
    fn minimize(
        ...
        exec: &mut E,
        input: &TxnTrace<EVMAddress, EVMAddress, ConciseEVMInput>,
        objective: &mut EVMIcyFeedback<'_>,
        corpus_id: usize,
    ) -> Vec<ConciseEVMInput> {
        let bug_meta = state.metadata::<BugMetadata>().unwrap();
        let bug_idx_needed = bug_meta
