...
use oracles::{
    erc20::IERC20OracleFlashloan, v2_pair::PairBalanceOracle,
};
...
pub fn evm_main(args: EvmArgs) {
    ...
    solution::init_cli_args(target, work_dir.clone(), &onchain);
    ...
    let flashloan_oracle = Rc::new(RefCell::new(IERC20OracleFlashloan::new(
    ...
    let mut state: EVMFuzzState = FuzzState::new(args.seed, work_dir);
    ...
    let offchain_artifacts = if !args.builder_artifacts_url.is_empty() {
        ...
        Some(
            OffChainArtifact::from_build_info_file(args.builder_artifacts_file)
                .expect("failed to parse builder artifacts"),
        )
    } else {
        None
