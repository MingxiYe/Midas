...
#[derive(Debug, Clone)]
pub struct EVMExecutor<I, S, VS, CI, SC>
where
...
impl<VS, I, S, CI, SC> EVMExecutor<I, S, VS, CI, SC>
where
    ...
    SC: Scheduler<State = S> + Clone + 'static,
{
    ...
    pub fn new(fuzz_host: FuzzHost<VS, I, S, SC>, deployer: EVMAddress) -> Self {
        Self {
            host: fuzz_host,
            deployer,
    ...
    fn execute_abi(
        ...
        #[cfg(feature = "real_balance")]
        {
            ...
            if tx_value > EVMU256::ZERO {
                let balance_caller = *vm_state
                    .get_balance(&input.get_caller())
                    .unwrap_or(&EVMU256::ZERO);
                let balance_contract = *vm_state
                    .get_balance(&input.get_contract())
                    .unwrap_or(&EVMU256::ZERO);
                if balance_caller < tx_value {
                ...
                vm_state.set_balance(input.get_caller(), balance_caller - tx_value);
                vm_state.set_balance(input.get_contract(), balance_contract + tx_value);
            }
        }
    ...
    pub fn reexecute_with_middleware(
        ...
        input: &I,
        state: &mut S,
        middleware: Rc<RefCell<dyn Middleware<VS, I, S, SC>>>,
    ) {
...
mod tests {
    ...
    fn test_fuzz_executor() {
        let mut state: EVMFuzzState = FuzzState::new(0, None); // nasty implementation, todo(@a3yip6)
        ...
        */
        ...
        let deployment_loc = evm_executor
            .deploy(
                ...
                None,
                generate_random_address(&mut state),
                &mut FuzzState::new(0, None), // nasty implementation, todo(@a3yip6)
            )
            .unwrap();
        ...
        let mut state = FuzzState::new(0, None); // nasty implementation, todo(@a3yip6)
