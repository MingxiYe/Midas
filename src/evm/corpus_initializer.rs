...
use crate::{
    evm::{
        ...
        onchain::{
            abi_decompiler::fetch_abi_heimdall, flashloan::register_borrow_txn, BLACKLIST_ADDR,
        },
        presets::Preset,
        types::{
            fixed_address, EVMAddress, EVMExecutionResult, EVMFuzzState, EVMInfantStateState,
            EVMStagedVMState, EVMU256,
        },
...
macro_rules! add_input_to_corpus {
    ($state: expr, $scheduler: expr, $input: expr) => {
        let mut testcase = wrap_input!($input);
        testcase.set_exec_time(Duration::from_secs(0));
        let idx = $state
            .add_tx_to_corpus(testcase.clone())
    ...
    ISC: Scheduler<State = EVMInfantStateState>,
{
        ...
        for contract in &loader.contracts {
            ...
            if let Some(srcmap) = &contract.raw_source_map {
                ...
                SOURCE_MAP_PROVIDER
                    .lock()
                    .unwrap()
                    .decode_instructions_for_address(
                        &contract.deployed_address,
                        runtime_bytecode,
                        srcmap.clone(),
                        &contract.files,
    ...
    fn add_abi(
        ...
        let input = EVMInput {
            ...
            sstate: StagedVMState::new_with_state(self.executor.host.evmstate.clone()),
            sstate_idx: 0,
