use std::{
    cell::RefCell, collections::HashMap, fs::File, io::Read, ops::Deref, path::Path, process::exit,
    rc::Rc,
};
...
use crate::{
    evm::{
        ...
        host::{
            FuzzHost, ACTIVE_MATCH_EXT_CALL, CALL_UNTIL, CMP_MAP, PANIC_ON_BUG, READ_MAP,
            TXN_WAYPOINT_MAP, WRITE_MAP, WRITE_RELATIONSHIPS,
        },
        ...
        oracles::{
            arb_call::ArbitraryCallOracle, echidna::EchidnaOracle, invariant::InvariantOracle,
            reentrancy::ReentrancyOracle, selfdestruct::SelfdestructOracle,
...
pub fn evm_fuzzer(
    ...
    let txn_waypoint = unsafe { &mut TXN_WAYPOINT_MAP };
    let cmps = unsafe { &mut CMP_MAP };
    let reads = unsafe { &mut READ_MAP };
    let writes = unsafe { &mut WRITE_MAP };
    let txn_waypoint_observer = unsafe { StdMapObserver::new("txn_waypoint", txn_waypoint) };
    ...
    let mut txn_waypoint_feedback = MaxMapFeedback::new(&txn_waypoint_observer);
    txn_waypoint_feedback
        .init_state(state)
    ...
    let mut executor =
        FuzzExecutor::new(evm_executor_ref.clone(), tuple_list!(txn_waypoint_observer));
    ...
    macro_rules! load_code {
        ($txn: expr) => {
            if let Some(onchain_mid) = onchain_middleware.clone() {
                if !onchain_mid.borrow_mut().loaded_abi.contains(&$txn.contract) {
        ...
        None => {
            ...
            for testcase in testcases {
                ...
                for txn in testcase {
                    ...
                    info!("================ Execution {} =================", idx);
                    let (mut inp, call_until) = txn.to_input(vm_state.clone());
                    inp.init_caller = Some(txn.caller);
                    unsafe {
                        CALL_UNTIL = call_until;
                    }
                    let (_, corpus_idx) = fuzzer
                    ...
                    // initialize the post state of historical transactions
                    // initialize the caller of historical transactions
                    if corpus_idx.is_some() {
                        let random_caller = state.get_rand_caller();
                        let mut testcase_mut = state.testcase_mut(corpus_idx.clone().unwrap()).unwrap();
                        let input_mut = testcase_mut.input_mut().as_mut().unwrap();
                        if state.get_execution_result().reverted {
                            debug!("The historical txn should not revert!");
                        } else {
                            input_mut.set_post_state(vm_state.clone().state);
                        }
                        input_mut.caller = random_caller;
                        input_mut.env.tx.caller = random_caller;
                    }
                }
            }
