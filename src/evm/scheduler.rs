    ...
    S: HasCorpus<Input = EVMInput> + HasTestcase + HasMetadata,
{
    fn on_add(&mut self, state: &mut Self::State, idx: CorpusId) -> Result<(), Error> {
        ...
        {
            let meta: &mut UncoveredBranchesMetadata = state
                .metadata_map_mut()
                .get_mut::<UncoveredBranchesMetadata>()
                .unwrap();
            ...
            for it in unsafe { BRANCH_STATUS.iter().take(BRANCH_STATUS_IDX) } {
                ...
                match meta.branch_status.get_mut(&(addr, pc)) {
                    ...
                    None => {
                        ...
                        meta.branch_status
                            .insert((addr, pc), BranchCoveredStatus::from(br));
...
pub type PowerABIMutationalStage<E, EM, I, M, Z> = PowerMutationalStageWithId<
    E,
    CorpusPowerABITestcaseScore<<E as UsesState>::State>,
    EM,
    I,
    M,
    Z,
