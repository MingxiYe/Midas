...
/// A trait for initializing the original profit (i.e., PostState) of inputs
pub trait HasPostState<VS>
where
    VS: Default + VMStateT,
{
    // Is the input initialized with original PostState
    fn is_post_state(&self) -> bool;

    // Set the initialized state
    fn set_post_state(&mut self, post_state: VS);
}
