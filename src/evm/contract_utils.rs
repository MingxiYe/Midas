    ...
    pub fn force_abi(&mut self, mapping: HashMap<String, String>) {
        for (filename_or_address, abi) in mapping {
            ...
            if filename_or_address.starts_with("0x") {
                addr = EVMAddress::from_str(&filename_or_address)
                    .map(Some)
                    .unwrap_or(None);
            }
...
impl ContractLoader {
    ...
    pub fn parse_abi_str(data: &str) -> Vec<ABIConfig> {
        let json: Vec<Value> = serde_json::from_str(&Self::normalize_abi_str(data))
            .expect("failed to parse abis file");
        json.iter()
            .flat_map(|abi| {
    ...
    pub fn from_address(
...

        {
            ...
            let block_number =
                EVMU256::from_str_radix(onchain.block_number.trim_start_matches("0x"), 16).unwrap();
        ...
        for addr in address {
            ...
            let (files, source_map_replacements, raw_source_map) =
                if let Some(job_result) = build_artifact.clone() {
                    (
    ...
    pub fn from_fork(
        ...
        for addr in targets {
            ...
            let (artifact_idx, slug) = Self::find_contract_artifact(
    ...
    pub fn from_setup(
        ...
        for (addr, code) in setup_data.clone().unwrap().code {
            let (artifact_idx, slug) =
                Self::find_contract_artifact(code.to_vec(), offchain_artifacts);
    ...
    fn get_vm_with_cheatcode(
        ...
        executor
            .host
            .add_middlewares(Rc::new(RefCell::new(Cheatcode::new())));
        (executor, state)
    }
    ...
    fn call_setup(deploy_code: Bytes, work_dir: String) -> SetupData {
        ...
        let (res, new_vm_state) = evm_executor.fast_call(
            &[(
                deployer,
                deployed_addr,
                Bytes::from_iter(calldata.iter().cloned()),
            )],
        ...
        macro_rules! parse_nth_result_selector {
            ($nth: expr) => {{
                ...
                if succ {
                    let sigs_parsed = ethers::abi::decode(
                        &[ethers::abi::ParamType::Array(Box::new(
                            ethers::abi::ParamType::Tuple(vec![
                                ethers::abi::ParamType::Address,
                                ethers::abi::ParamType::Array(Box::new(
                                    ethers::abi::ParamType::FixedBytes(4),
                                )),
                            ]),
                        ))],
                        &res_bys,
                    ...
                    let sigs_parsed =
                        if let ethers::abi::Token::Array(sigs_parsed) = sigs_parsed[0].clone() {
                            sigs_parsed
                        } else {
                            panic!("invalid array")
                        };
                    ...
                    sigs_parsed.iter().for_each(|x| {
                        if let ethers::abi::Token::Tuple(tuple) = x {
                            ...
                            if let (
                                ...
                                ethers::abi::Token::Array(selectors),
                            ) = (addr, selectors)
                            {
