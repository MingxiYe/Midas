...
use crate::{
    ...
    evm::{
        abi::get_abi_type_boxed,
        input::{EVMInput, EVMInputT, EVMInputTy},
        onchain::AccessPattern,
        types::{convert_u256_to_h160, EVMAddress, EVMU256},
        uniswap::{
            get_uniswap_info, PairContext, PathContext, UniswapProvider, UniswapTokenContext,
        },
        ContractLoader,
    },
    input::VMInputT,
...
impl OnChainConfig {
    ...
    pub fn fetch_transaction_by_hash(&mut self, tran_hash: String) -> Option<EVMInput> {
        let resp = {
            let mut params = String::from("[");
            params.push_str(&format!("\"{}\"", tran_hash));
            params.push_str("]");
            self._request("eth_getTransactionByHash".to_string(), params)
        };

        match resp {
            Some(resp) => {
                let caller = EVMAddress::from_str(resp["from"].as_str().unwrap_or_default()).unwrap_or_default();
                let contract = EVMAddress::from_str(resp["to"].as_str().unwrap_or_default()).unwrap_or_default();
                if contract == EVMAddress::default() {
                    return None;
                }
                let mut txn = EVMInput {
                    caller,
                    contract,
                    data: None,
                    ...
                    sstate_idx: 0,
                    txn_value: Some(
                        EVMU256::from_str(resp["value"].as_str().unwrap_or_default()).unwrap_or_default(),
                    ),
                    step: false,
                    env: Default::default(),
                    ...
                    post_state: None,
                    init_caller: Some(caller),
                };
                txn.get_vm_env_mut().tx.caller = caller;
                txn.get_vm_env_mut().block.number =
                    EVMU256::from_str(resp["blockNumber"].as_str().unwrap_or_default()).unwrap_or(EVMU256::from(1))
                        - EVMU256::from(1);

                // init call & contract balance
                #[cfg(feature = "real_balance")]
                {
                    txn.get_state_mut()
                        .set_balance(caller, self.get_balance(caller));
                    txn.get_state_mut()
                        .set_balance(contract, self.get_balance(contract));
                }

                let abi = self.fetch_abi(txn.contract);
                let txn_data = resp["input"].as_str().unwrap_or_default();
                if txn_data.len() <= 10 {
                    return None;
                }
                let txn_func = hex::decode(&txn_data[2..10]).expect("Decoding failed");
                let txn_para = hex::decode(&txn_data[2..]).expect("Decoding failed");

                txn.direct_data = Bytes::from(txn_para.clone());
                if abi.is_some() {
                    let abis = ContractLoader::parse_abi_str(&abi.unwrap());
                    for abi in abis {
                        if abi.function != &txn_func[..] {
                            continue;
                        }
                        ...
                        abi_instance.set_bytes(txn_para.clone());
                        txn.data = Some(abi_instance);
                        break;
                    }
                }
                Some(txn)
...
impl OnChainConfig {
    fn get_pair(&mut self, token: &str, network: &str, is_pegged: bool) -> Vec<PairData> {
        if let Ok(result) = self.get_pair_inner(token, network, is_pegged) {
            result
        } else {
            vec![]
        }
    ...
    fn get_pair_inner(&mut self, token: &str, network: &str, is_pegged: bool) -> Result<Vec<PairData>, Box<dyn std::error::Error>> {
        ...
        {
            return Ok(self.pair_cache[&EVMAddress::from_str(&token).unwrap()].clone());
        }
    ...
    fn fetch_reserve(&self, pair: &str) -> (String, String) {
        ...
        if result.len() != 196 {
            ...
            let pair_code = self
                .clone()
                .get_contract_code(B160::from_str(pair).unwrap(), true);
    ...
    fn add_reserve_info(&self, pair_data: &mut PairData) -> bool {
        ...
        let reserves_0 =
            EVMU256::from(i128::from_str_radix(&pair_data.initial_reserves_0, 16).unwrap());
        let reserves_1 =
            EVMU256::from(i128::from_str_radix(&pair_data.initial_reserves_1, 16).unwrap());
