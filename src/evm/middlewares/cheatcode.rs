...
use revm_interpreter::{
    analysis::to_analysed, opcode, BytecodeLocked, InstructionResult, Interpreter,
};
    ...
    SC: Scheduler<State = EVMFuzzState> + Clone,
{
    ...
    pub fn cheat_call(&mut self, interp: &mut Interpreter, host: &mut FuzzHost<SC>) {
        ...
        let res = match vm_call {
            ...
            VmCalls::expectCallMinGas_0(args) => {
                self.expect_call_mingas0(&mut host.expected_calls, args)
            }
            VmCalls::expectCallMinGas_1(args) => {
                self.expect_call_mingas1(&mut host.expected_calls, args)
            }
    ...
    SC: Scheduler<State = EVMFuzzState> + Clone,
{
    ...
    fn addr(&self, args: Vm::addrCall) -> Option<Vec<u8>> {
        ...
        let address: Address = privateKey.to_be_bytes::<{ U256::BYTES }>()[..20]
            .try_into()
            .unwrap();
...
unsafe fn pop_cheatcall_stack(
    interp: &mut Interpreter,
    op: u8,
) -> Result<(Bytes, usize, usize), InstructionResult> {
    // Pop gas, addr, val
...
fn peek_realcall_target(interp: &Interpreter) -> Result<Address, InstructionResult> {
    ...
    let addr: Address = addr_bytes.to_be_bytes::<{ U256::BYTES }>()[12..]
        .try_into()
        .unwrap();
    Ok(addr)
}
...
fn peek_realcall_input_value(
    interp: &mut Interpreter,
    op: u8,
) -> Result<(Bytes, U256), InstructionResult> {
...
fn peek_log_data(interp: &mut Interpreter) -> Result<AlloyBytes, InstructionResult> {
    ...
    Ok(AlloyBytes::copy_from_slice(
        interp.memory.get_slice(offset, len),
    ))
}
...
fn handle_expect_emit(
    ...
    if expected_topic_0.zip(log_topic_0).map_or(false, |(a, b)| {
        a == b && expected.topics().len() == topics.len()
    }) {
        // Match topics
...
mod tests {
    ...
    fn test_foundry_contract() {
        let mut state: EVMFuzzState = FuzzState::default();
        ...
        let mut deploy_state = FuzzState::default();
        // Deploy Reverter
        let _ = evm_executor
        ...
        macro_rules! assert_fn_success {
            ($fn_selector:expr) => {
                ...
                input
                    .sstate
                    .state
                    .set_balance(cheat_test_addr, U256::from(1000));
    ...
    fn load_bytecode(path: &str) -> Bytecode {
        let hex_code = fs::read_to_string(path)
